...
use crate::helix::theme::palette::{keysym, literal, punc};
...
mod palette {
    ...
    pub fn normal() -> Chord {
        Chord::from(Vector3::new(0.805, 0.06, 0.20)).set_interval([1.08, 0.07, -0.03].into())
    }

    pub fn punc() -> Chord {
        normal().mk_red().shift_sat(0.14).push_back().push_back()
    }

    pub fn keysym() -> Chord {
        normal().mk_saturated().mk_green().pin_bottom(&normal())
    }

    pub fn literal() -> Chord {
        normal().mk_saturated().mk_yellow().pin_bottom(&normal())
    }
    ...
    node("")
        ...
        .child(
            node("constant")
                ...
                .child(
                    node("numeric")
                        .transform(|c| c.mix(&literal()).pin_bottom(c))
                        .child(node("integer"))
                        .child(node("float")),
                ),
        )
        ...
        .child(
            node("variable")
                .child(node("builtin").mask_bg().transform(|c| c.alt(11)))
                .child(node("parameter"))
                .child(node("other").child(node("member").child(node("private")))),
        )
        .child(node("label"))
        .child(keyword())
        .child(node("operator").mask_bg().transform(|c| c.push_back()))
        .child(
            node("function")
        ...
        .child(
            node("comment")
                .transform(|c| c.desaturated().push_back().push_back().pin_bottom(c))
        ...
        .child(
            node("punctuation")
                ...
                .mask_fg()
                .child(node("delimiter").transform(|c| c.alt(80)))
                .child(node("bracket"))
                .child(node("special")),
        )
        .child(
...
fn ui() -> Node {
    node("ui")
        ...
        .child({
            node("selection")
                .mask_fg()
                .transform(|c| c.mk_blue())
                .child(node("primary").mask_fg())
        })
        .child(
            node("highlight")
        ...
        .child(
            node("text")
                ...
                .child(
                    node("directory")
                        .mask_bg()
                        .transform(|c| c.push_back().pin_bottom(c)),
                )
...
fn virtualx() -> Node {
    node("virtual")
        ...
        .child(
            node("whitespace")
                .mask_bg()
                .transform(|c| c.faintly().pin_bottom(&palette::normal())),
        )
        .child(
            node("indent-guide")
                .mask_bg()
                .transform(|c| c.mk_void().pin_bottom(&palette::normal())),
        )
        .child(
            node("inlay-hint")
                .child(node("parameter"))
